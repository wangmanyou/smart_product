    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            title="确认删除该角色？"
            description={record.isUsed ? '该角色还有关联用户，请先调整用户角色。' : '删除后角色权限配置会一并删除。'}
            disabled={record.isBuiltin || record.isUsed}
            onConfirm={async () => {
          ...
          >
            <Button type="link" danger disabled={record.isBuiltin || record.isUsed} icon={<DeleteOutlined />}>
              删除
            </Button>
          </Popconfirm>
        </Space>
