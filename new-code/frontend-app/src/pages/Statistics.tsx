...
export default function Statistics() {
  ...
  const hasFilters = Boolean(range) || sceneNames.length > 0;
  ...
  const filteredRows = useMemo(
    () => {
      if (!sceneNames.length) return rows;
      const selected = new Set(sceneNames);
      return rows.filter((item) => selected.has(item.sceneName || '未命名场景'));
    },
    [rows, sceneNames],
  );
    ...
    <PageHeader
      ...
      extra={[
        ...
        <Select
          ...
          maxTagCount={0}
          maxTagPlaceholder={() => {
            const selectedText = sceneNames.join('、');
            return (
              <span className="dashboard-scene-overflow" title={selectedText}>
          ...
          placeholder="全部场景"
          onChange={setSceneNames}
        />,
        <Button key="reset" icon={<UndoOutlined />} onClick={resetFilters} disabled={!hasFilters} loading={loading}>
          重置
        </Button>,
      ]}
    >
